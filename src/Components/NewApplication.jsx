...
function validateMPAN(mpan) {
  ...
  if (!/^\d{21}$/.test(digits)) return { valid: false, error: 'MPAN must be 21 digits' }
    ...
    <div className="max-w-2xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-gray-200 p-6 space-y-5">
        ...
        <div>
          ...
          <input type="text" name="site_address" value={form.site_address} onChange={handleChange}
        ...
        <div>
          ...
          <div className="relative">
            <input type="text" name="postcode" value={form.postcode} onChange={handleChange}
              placeholder="e.g. BB11 1AA" required maxLength={8}
          ...
          {dnoStatus === 'error' && <p className="mt-1 text-xs text-red-500">Postcode not recognised — please check and try again.</p>}
        </div>

        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">MPAN Number <span className="text-gray-400 font-normal">(21 digits — optional)</span></label>
          <input type="text" name="mpan" value={form.mpan} onChange={handleChange}
            placeholder="21 digit MPAN number" maxLength={21}
          ...
          {mpanCheck && (
            <p className={`mt-1 text-xs ${mpanCheck.valid ? 'text-green-600' : 'text-red-500'}`}>
